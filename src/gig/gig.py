...
class Server:
    def __init__(self, server_url, timeout=30):
        ...
        if test_connection == 200:
            ...
            raise SystemError("Error: unable to reach server")
    ...
    def terminate(self, entity):
        ...
        if response.status_code not in [200, 202]:
            raise SystemError('Entity termination error', json.loads(response.content))
        return response.status_code
    ...
    def normalize_name(self, search_text):
        ...
        if response.status_code != 200:
            raise FileNotFoundError('No result found', json.loads(response.content))
